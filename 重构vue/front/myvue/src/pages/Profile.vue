  ...
  <div class="profile-container">
    ...
    <div class="profile-card">
      <div class="avatar-section">
        <img :src="userInfo.avater || '/img/default.png' " class="avatar-img" @click="changeAvatar" alt="头像" />
...
export default {
  ...
  try {
    ...
    const res = await axios.get(`https://ae88-2001-250-4001-208-addd-61f-1192-be83.ngrok-free.app/api/user/profile?username=${username}`, {
    ...
    async onAvatarSelected(e) {
      ...
      reader.onload = async (evt) => {
        this.userInfo.avater = evt.target.result;
        // 上传到后端
        const formData = new FormData();
        formData.append('avatar', file);
        formData.append('username', this.userInfo.username); // 🔥 添加用户名到表单数据
        try {
          const res = await axios.post('https://ae88-2001-250-4001-208-addd-61f-1192-be83.ngrok-free.app/api/user/avatar', formData, {
          ...
          if (res.data.code === 0) {
            ...
            this.userInfo.avater = res.data.avatar; // 使用返回的头像地址
            // 🔥 更新Vuex和localStorage都使用服务器路径
                const updatedUserInfo = { ...this.userInfo, avater: res.data.avatar };
                this.$store.commit('user/SET_USER_INFO', updatedUserInfo);
                
                const currentUserData = JSON.parse(localStorage.getItem('userInfo') || '{}');
                const updatedUserData = { ...currentUserData, avater: res.data.avatar };
                localStorage.setItem('userInfo', JSON.stringify(updatedUserData));
    ...
    async updateSignature() {
      try {
        const res = await axios.post('https://ae88-2001-250-4001-208-addd-61f-1192-be83.ngrok-free.app/api/user/signature', {
